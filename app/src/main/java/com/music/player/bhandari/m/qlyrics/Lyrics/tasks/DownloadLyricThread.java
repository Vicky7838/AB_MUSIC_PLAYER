...
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.AZLyrics;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.Bollywood;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.Genius;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.JLyric;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.Lololyrics;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.LyricWiki;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.Lyrics;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.LyricsMania;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.MetalArchives;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.PLyrics;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.UrbanLyrics;
import com.music.player.bhandari.m.qlyrics.Lyrics.lyrics.ViewLyrics;
import com.music.player.bhandari.m.qlyrics.Lyrics.offlineStorage.DbHelperLyrics;
...
public class DownloadLyricThread extends Thread {
    ...
    public DownloadLyricThread(final Lyrics.Callback callback, boolean positionAvailable, TrackItem item, final String... params) {
        super(DownloadLyricThread.getRunnable(callback, positionAvailable,item, params));
        //DownloadLyricThread.item =item;
    }

    public static void setProviders(Iterable<String> providers) {
        DownloadLyricThread.providers = new ArrayList<>(Arrays.asList(mainProviders));
        for (String provider : providers) {
            if (provider.equals("ViewLyrics"))
                DownloadLyricThread.providers.add(0, provider);
            else
                DownloadLyricThread.providers.add(provider);
        }
    }

    public static Runnable getRunnable(final Lyrics.Callback callback, final boolean positionAvailable,
                                       final TrackItem item, final String... params) {
        return new Runnable() {
            ...
            private void threadMsg(Lyrics lyrics) {
                if (lyrics != null) {
                    ...
                    if(item!=null && lyrics.getFlag()==Lyrics.POSITIVE_RESULT) {
                        ...
                        OfflineStorageLyrics.putLyrics(lyrics, item);
                    }
